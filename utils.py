...
def get_loader(data_name, img_size=256, batch_size=8, split='test',
    ...
    if dataset == 'CDDataset':
        data_set = CDDataset(root_dir=root_dir, split=split, img_size=img_size, is_train=True,
...
def get_loaders(args):
    ...
    #                         label_transform=label_transform)
    # elif args.dataset == 'MITCDDataset':
    #     training_set = MITCDDataset(root_dir=root_dir, split=split,
    #                                 img_size=args.img_size, is_train=True,
    #                                 label_transform=label_transform)
    #     val_set = MITCDDataset(root_dir=root_dir, split=split_val,
    #                            img_size=args.img_size, is_train=False,
    #                            label_transform=label_transform)
    if args.dataset == 'CDDataset':
        training_set = CDDataset(root_dir=root_dir, split=split,
    ...
    #     train_loader = DataLoader(training_set, batch_size=args.batch_size, sampler=train_sampler, pin_memory=True,
    #                               drop_last=True)
