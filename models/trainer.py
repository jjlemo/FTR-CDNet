...
from models.losses import DiceLoss, FocalLoss, cross_entropy
...
class CDTrainer():
    def __init__(self, args, dataloaders):
        ...
        # define G
        self.net_G = networks.define_G(args=args, gpu_ids=args.gpu_ids)
        ...
        self.exp_lr_scheduler_G = networks.get_scheduler(self.optimizer_G, args)
        ...
        if args.loss == 'ce':
            ...
            self._loss_1 = FocalLoss(torch.nn.BCEWithLogitsLoss(), gamma=2)
            self._loss_2 = cross_entropy
        else:
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            pretrained_dict = torch.load(self.args.pretrain)
    ...
    def _forward_pass(self, batch):
        ...
        self.G_pred = self.net_G(img_in1, img_in2, img_in3)
    ...
    def _backward_G(self):
        ...
        if self.multi_scale_train == "True":
            ...
            self.G_loss = self._pxl_loss(self.G_pred[-1], gt) + self._loss_1(self.G_pred[-1], gt) + self._loss_2(
                self.G_pred[-1], gt)
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            # Iterate over data.
            total_val = len(self.dataloaders['val'])
            for self.batch_id, batch in tqdm(enumerate(self.dataloaders['val'], 0), total=total_val):
                with torch.no_grad():
                    self._forward_pass(batch)
                self._collect_running_batch_states()
            self._collect_epoch_states()
