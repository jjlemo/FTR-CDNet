...
class MITCDEvaluator():
    ...
    def _forward_pass(self, batch):
        ...
        img_in1 = batch['A'].to(self.device)
        img_in2 = batch['B'].to(self.device)
        p_img_in1 = batch['PA'].to(self.device)
        p_img_in2 = batch['PB'].to(self.device)
        self.shape_h = img_in1.shape[-2]
        self.shape_w = img_in1.shape[-1]
        self.G_pred = self.net_G(img_in1, img_in2, p_img_in1, p_img_in2)
        return self._visualize_pred()

    def predict_pass(self, img_in1, img_in2, p_img_in1, p_img_in2):
        img_in1 = img_in1.to(self.device).unsqueeze(0)
        img_in2 = img_in2.to(self.device).unsqueeze(0)
        p_img_in1 = p_img_in1.to(self.device).unsqueeze(0)
        p_img_in2 = p_img_in2.to(self.device).unsqueeze(0)
        self.G_pred = self.net_G(img_in1, img_in2, p_img_in1, p_img_in2)
        return self._visualize_pred()
...
class TIPCDEvaluator():
    ...
    def predict_pass(self, img_in1, img_in2, img_in3):
        img_in1 = img_in1.to(self.device).unsqueeze(0)
        img_in2 = img_in2.to(self.device).unsqueeze(0)
        img_in3 = img_in3.to(self.device).unsqueeze(0)
        self.G_pred = self.net_G(img_in1, img_in2, img_in3)
        return self._visualize_pred()
